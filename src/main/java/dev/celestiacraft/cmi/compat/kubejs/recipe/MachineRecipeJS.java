...
public class MachineRecipeJS extends RecipeJS {
	...
	public MachineRecipeJS outputItem(Object... items) {
		for (Object item : items) {
			...
			if (!output.isEmpty()) {
				outputItems.add(new MachineRecipe.ItemOutput(output.item, output.hasChance() ? output.getChance() : 1.0D));
			}
		}
		save();
		return this;
	...
	public MachineRecipeJS outputFluid(Object... fluids) {
		for (Object fluid : fluids) {
			FluidStackJS stack = FluidStackJS.of(fluid);
			if (!stack.kjs$isEmpty()) {
				outputFluids.add(new MachineRecipe.FluidOutput(toForgeStack(stack), stack.hasChance() ? stack.getChance() : 1.0D));
			}
		}
		save();
		return this;
